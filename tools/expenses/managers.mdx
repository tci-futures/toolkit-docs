...
- Sign in to the Toolkit and open the [Expenses app](https://tools.tcifutures.co.uk/admin/finance/expenses/settings).
- Confirm you are listed as a manager, which you can check on the settings page.

At the bottom of the Settings screen there is a Team Members table which lists all users who currently have you set as their manager. Use this table to verify who will appear in your Approvals view.

{/* add an image from /images/expenses-approvals.png */}
![Manager Approvals](/images/expenses-approvals.png)
...
<Step title="Open Approvals">
	Go to the Expenses app and open the Manager Approvals view to see employee expenses.
</Step>
...
## Check the status of a user's expenses

![Manager Approvals](/images/expenses-approvals-selected.png)

Managers can view the current status of each employee's expenses at the user level.

<Steps>
	<Step title="Open a user's expenses">
		Click the Eye icon next to the relevant expense type to open that user's
		list of expenses for that type.
	</Step>

    <Step title="Read the status">
    	The table displays every expense for that user and expense type, which by default is filtered to only show expenses you need to review.
    </Step>

    <Step title="Filter by status">
    	Use the status filter above the table to show items in a specific state
    	(for example: Approved). This is the best way to confirm which items have
...
If you have turned on "Smart Notifications", you will receive them roughly daily when there are expenses that need your attention. Smart Notifications are designed to surface only the employees or expense groups that have new items.
...
Managers receive a monthly reminder email (typically sent around the 4th of each month) summarising outstanding items that still need approval.

There is also a separate monthly reminder sent to users who have created expenses but not yet submitted them to their manager — this helps prompt people to finish and send claims for review.
